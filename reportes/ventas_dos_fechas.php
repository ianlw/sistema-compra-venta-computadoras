...
$ventas_diarias_result = null;
$fecha_inicio = isset($_POST['fecha_inicio']) ? $_POST['fecha_inicio'] : null;
$fecha_fin = isset($_POST['fecha_fin']) ? $_POST['fecha_fin'] : null;

// Consulta para obtener el reporte de ventas diarias
...
if ($stmt = $conn->prepare($ventas_diarias_query)) {
    $stmt->bind_param("ss", $fecha_inicio, $fecha_fin);
    $stmt->execute();
    $ventas_diarias_result = $stmt->get_result();
} else {
    die("Error en la consulta de ventas diarias: " . $conn->error);
}
?>
...
<head>
    ...
    <title>Reporte de Ventas Diarias - Sistema de Compra y Venta</title>
...
<body class="bg-gray-100">
    ...
    <main class="p-4">
        <h2 class="text-xl font-bold mb-4">Ventas Diarias</h2>
            ...
            <table class="w-full mt-4 border-collapse border border-gray-300">
                <thead>
                    <tr>
                        ...
                        <th class="border border-gray-300 px-4 py-2">Cantidad de Ventas</th>
                        <th class="border border-gray-300 px-4 py-2">Productos Vendidos</th>
                        <th class="border border-gray-300 px-4 py-2">Total de Ventas</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if ($ventas_diarias_result && $ventas_diarias_result->num_rows > 0) : ?>
                        <?php while ($row = $ventas_diarias_result->fetch_assoc()) : ?>
                            <tr>
                                <td class="border border-gray-300 px-4 py-2"><?php echo htmlspecialchars($row['fecha']); ?></td>
                                <td class="border border-gray-300 px-4 py-2"><?php echo htmlspecialchars($row['cantidad_ventas']); ?></td>
                                <td class="border border-gray-300 px-4 py-2"><?php echo htmlspecialchars($row['cantidad_productos_vendidos']); ?></td>
                                <td class="border border-gray-300 px-4 py-2"><?php echo number_format(htmlspecialchars($row['total_ventas']), 2); ?></td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else : ?>
                        <tr>
